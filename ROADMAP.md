...
**On disk that factor does not survive, and the measurement says so.** Running
the search over the same simulated segment twice, once writing each format:

| | dense `wt*` | sparse pairs | |
|---|---|---|---|
| H1 foreground, 153 041 triggers | 49.4 MB | 15.7 MB | **3.1×** |

Parquet had already taken most of the 130×, because a column of exact zeros
encodes to almost nothing, while a scalar column of incompressible floats does
not. What is left is a factor of three, and dropping the reconstructed waveform
adds to it wherever a run wrote `rw*`, since those never compressed at all.

The size argument is therefore a real but modest one, and the reason to make the
change is the other one below. Measure this by writing both formats from the
search, not by re-encoding one file into the other: a re-written file lands its
rows in different row groups and compresses differently, which understates the
difference by more than half.
